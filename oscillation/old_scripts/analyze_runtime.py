...
# 
    ...
    # Full circuit - all activations
    activations=[[0, 0], [0, 1], [0, 2], [1, 0], [1, 1], [1, 2], [2, 0], [2, 1], [2, 2]]
    inhibitions=[]
    
    nc = 3
    Am, Rm, U = make_matrices_for_ssa(
        nc, activations=activations, inhibitions=inhibitions
    )
    ssa = GillespieSSA(
        seed=0,
        n_species=nc,
    ...
    if save:
        fpath = Path(save_dir).joinpath(
            f"2023-06-27_random_sample_runtimes_worstcase_circuit.csv"
        )
...
if __name__ == "__main__":
    ...
    main(save_dir=save_dir, n_samples=2500, n_replicates=1, n_threads=None, save=True)
