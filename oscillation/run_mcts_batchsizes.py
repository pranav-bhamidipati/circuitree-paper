# A script that runs MCTS using a transposition table dataset
# Runs are performed with n = 5 replicates (different random seeds)
# Each replicate performs MCTS up to some large number of visits
# This is repeated for various batch sizes (number of samples per visit)
#   - 1, 5, 10, 50, 100, 500
# It should store ?? for every step
#   - sampled node
#   - Reward
#   - simulated node
#   - modularity estimate
# Then for each (terminal) circuit at each time-point, does it have an "oscillator" phenotype?
#   - What criterion to use?
#       - Cumulative reward will mean that it's monotonic. But at high sample numbers, you
#           will start to capture more false positives. So it's pretty unclear how to
#           decide the "right" threshold to set and how long to run.
#    ** - Average reward will be (a bit?) squiggly but makes more sense bc it will be
#           interpretable at all time-points and will converge to the true phenotype.
#   - At first, just use the time-points where a terminal circuit is sampled, even though that
#       will mean skipping many of the early time-points
#   - If that doesn't work (too much skipping), then plot the sampled circuit, terminal or not
...
from models.oscillation.oscillation_parallel import OscillationTreeParallel
