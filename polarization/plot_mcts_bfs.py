...
def plot_complexity_graph(
    ...
    **kwargs,
    ...
    unique_genotypes = np.array([PolarizationTree.get_unique_state(n) for n in G.nodes])
    ...
    colors = np.array(
        [win_probabilities.get(PolarizationTree.get_edge_code(n), 0.0) for n in G.nodes]
    )
